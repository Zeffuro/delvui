...
namespace DelvUI.Interface.PartyCooldowns
{
    ...
    public class PartyCooldownsDataConfig : PluginConfigObject
    {
        ...
        public static Dictionary<uint, PartyCooldownData> DefaultCooldowns = new Dictionary<uint, PartyCooldownData>()
        {
            ...
            [7405] = NewData(7405, JobIDs.BRD, 62, 90, 15, 70, 2, true, new TraitEnhanced {CooldownDuration = 90, Level = 88}),   // troubadour
            [7408] = NewData(7408, JobIDs.BRD, 66, 90, 15, 40, 2),                                                                // nature's minne
            [25785] = NewData(25785, JobIDs.BRD, 90, 110, 15, 30, 3),                                                             // radiant finale
            [16012] = NewData(16012, JobIDs.DNC, 56, 90, 15, 70, 2, true, new TraitEnhanced {CooldownDuration = 90, Level = 88}), // shield samba
            [16004] = NewData(16004, JobIDs.DNC, 70, 120, 20, 30, 3),                                                             // technical step / finish
            [16889] = NewData(16889, JobIDs.MCH, 56, 90, 15, 70, 2, true, new TraitEnhanced {CooldownDuration = 90, Level = 88}), // tactician
        ...
        #region helpers
        private static PartyCooldownData NewData(uint actionId, uint jobId, uint level, int cooldown, int effectDuration, int priority, int column, bool enabled = true, TraitEnhanced? traitEnhanced = null)
        {
            PartyCooldownData data = NewData(actionId, level, cooldown, effectDuration, priority, column, enabled);
            data.JobId = jobId;
            data.Role = JobRoles.Unknown;
            data.TraitEnhanced = traitEnhanced;

            return data;
        }

        private static PartyCooldownData NewData(uint actionId, JobRoles role, uint level, int cooldown, int effectDuration, int priority, int column, bool enabled = true, TraitEnhanced? traitEnhanced = null)
        {
            PartyCooldownData data = NewData(actionId, level, cooldown, effectDuration, priority, column, enabled);
            data.JobId = 0;
            data.Role = role;
        ...
        private static PartyCooldownData NewData(uint actionId, List<JobRoles> roles, uint level, int cooldown, int effectDuration, int priority, int column, bool enabled = true, TraitEnhanced? traitEnhanced = null)
        {
            PartyCooldownData data = NewData(actionId, level, cooldown, effectDuration, priority, column, enabled);
            data.JobId = 0;
            data.Role = JobRoles.Unknown;
        ...
        private static PartyCooldownData NewData(uint actionId, uint level, int cooldown, int effectDuration, int priority, int column, bool enabled = true, TraitEnhanced? traitEnhanced = null)
        {
